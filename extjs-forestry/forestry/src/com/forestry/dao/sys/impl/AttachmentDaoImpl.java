...
public class AttachmentDaoImpl extends BaseDao<Attachment> implements AttachmentDao {
	...
	public List<Object[]> queryFlowerList(String epcId) {
		Query query = this
				.getSession()
				.createSQLQuery(
						"select ft.name,group_concat(a.file_path),ft.description,f.epc_id from forestry_type ft inner join forestry f on ft.id=f.type_id left join attachment a on a.forestrytype_id=ft.id where f.epc_id=? group by a.file_name");
