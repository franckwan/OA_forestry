...
public class SensorDataDaoImpl extends BaseDao<SensorData> implements SensorDataDao {
	...
	public List<Object[]> doGetSensorDataStatistics(Short sensorType) {
		...
		// Oracle
		// Query query =
		// getSession().createSQLQuery("select concat(to_char(record_time, 'yyyy-mm-dd hh24'),':00') name,round(avg(sensor_value),2) data1,max(sensor_value) mx,min(sensor_value) mn,wm_concat(sensor_value) gc from sensor_data sd where sensor_type = ? group by to_char(record_time, 'yyyy-mm-dd hh24')");
		// SQL Server
		// Query query =
		// getSession().createSQLQuery("select CONVERT(varchar(12) ,record_time, 114) name,round(avg(sensor_value),2) data1,max(sensor_value) mx,min(sensor_value) mn,'' gc from sensor_data sd where sensor_type = ? group by CONVERT(varchar(12) ,record_time, 114)");
